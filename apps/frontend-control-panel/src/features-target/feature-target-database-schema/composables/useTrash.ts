...
export function useTrash() {
    ...
    const fetchTrash = useCallback(async () => {
        setLoading(true);
        try {
            const response = await apiClient.get<DatabaseTable[]>(`${api.databaseSchema.list}?archived=true`);
            if (response.status === API_STATUS.SUCCESS) {
                setItems(response.data || []);
            }
        } catch {
            addToast(msg.databaseSchema.error.loadFailed, TOAST_TYPE.ERROR);
        } finally {
            setLoading(false);
        }
    ...
    const restore = useCallback(async (id: number) => {
        try {
            const response = await apiClient.post(api.databaseSchema.restore(id));
            if (response.status === API_STATUS.SUCCESS) {
    ...
    const destroy = useCallback(async (id: number) => {
        try {
            ...
            const response = await apiClient.delete(`${api.databaseSchema.list}/${id}/destroy`);
            if (response.status === API_STATUS.SUCCESS) {
                addToast(msg.databaseSchema.success.sourceDeleted, TOAST_TYPE.SUCCESS); // Use generic delete message or specific if available
                fetchTrash();
                return true;
            }
        } catch {
