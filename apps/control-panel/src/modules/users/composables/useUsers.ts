...
export function useUsers() {
    ...
    const createUser = useCallback(async (formData: UserFormData) => {
        setSubmitting(true);
        try {
            const res = await fetchWithCsrf(USERS_ENDPOINTS.create, {
                method: 'POST',
    ...
    const updateUser = useCallback(async (id: number, formData: Partial<UserFormData>) => {
        setSubmitting(true);
        try {
            const res = await fetchWithCsrf(USERS_ENDPOINTS.update(id), {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify(formData)
            });
    ...
    const deleteUser = useCallback(async (id: number) => {
        setSubmitting(true);
        try {
            const res = await fetchWithCsrf(USERS_ENDPOINTS.delete(id), { method: 'DELETE' });
            const data = await res.json();
            if (data.status === 'success') {
