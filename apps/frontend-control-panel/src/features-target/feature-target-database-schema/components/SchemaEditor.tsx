...
import { Button, Badge, Card, CardContent, Alert, AlertTitle, AlertDescription, Empty, EmptyHeader, EmptyMedia, EmptyTitle, PageTitle, TextHeading } from '@/components/ui';
...
export const SchemaEditor = ({ DatabaseTableId }: SchemaEditorProps) => {
    ...
    return (
        <TargetLayout>
            <div className="flex flex-col gap-10 animate-page-enter max-w-5xl mx-auto pb-10">
                {/* Header Section */}
                <header className="flex flex-col gap-6 px-1">
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6">
                        <PageTitle
                            ...
                            subtitle={
                                <>
                                    editing table structure for <Badge variant="secondary">{source?.tableName}</Badge>
                                </>
                            }
                        />
                ...
                <div className="space-y-4">
                    <div className="flex items-center gap-2">
                        <Icons.config className="text-muted-foreground/40" />
                        <p className="text-lg font-medium text-muted-foreground">structure definition</p>
                    </div>
                    <ColumnBuilder
                        columns={columns}
                        onChange={setColumns}
