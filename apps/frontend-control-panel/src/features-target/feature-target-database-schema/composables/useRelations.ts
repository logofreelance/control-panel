...
export function useRelations(DatabaseTableId: number) {
    ...
    const fetchRelations = useCallback(async () => {
        try {
            ...
            const response = await apiClient.get<Relation[]>(api.databaseSchema.relations(DatabaseTableId));
    ...
    const fetchTargets = useCallback(async () => {
        try {
            // ✅ Use api from context
            const response = await apiClient.get<RelationTarget[]>(api.databaseSchema.availableTargets(DatabaseTableId));
            if (response.status === API_STATUS.SUCCESS && response.data) {
    ...
    const addRelation = useCallback(async (payload: AddRelationPayload): Promise<Relation | null> => {
        ...
        try {
            // ✅ Use api from context
            const response = await apiClient.post<Relation>(api.databaseSchema.addRelation(DatabaseTableId), payload);
            ...
            if (response.status === API_STATUS.SUCCESS) {
                ...
                addToast((response as { message?: string }).message || msg.databaseSchema.error.relationFailed, TOAST_TYPE.ERROR);
            }
        } catch {
    ...
    const deleteRelation = useCallback(async (relationId: string | number): Promise<boolean> => {
        ...
        try {
            ...
            const response = await apiClient.delete(api.databaseSchema.deleteRelation(DatabaseTableId, Number(relationId)));
            if (response.status === API_STATUS.SUCCESS) {
    ...
    const updateRelation = useCallback(async (relationId: string | number, payload: { alias?: string, type?: string }): Promise<boolean> => {
        try {
            ...
            const response = await apiClient.put(api.databaseSchema.updateRelation(DatabaseTableId, Number(relationId)), payload);
            if (response.status === API_STATUS.SUCCESS) {
                addToast(msg.databaseSchema.success.relationUpdated || "Relation updated", TOAST_TYPE.SUCCESS);
                await fetchRelations();
                return true;
            } else {
                addToast((response as { message?: string }).message || msg.databaseSchema.error.relationFailed, TOAST_TYPE.ERROR);
