...
export const useApiTester = (): UseApiTesterReturn => {
    ...
    const [url, setUrl] = useState(`${env.BACKEND_SYSTEM_API}/`);
    ...
    useEffect(() => {
        ...
        if (queryPath) {
            // If path is full URL, use it directly. Otherwise assume relative to BACKEND_SYSTEM_API
            if (queryPath.startsWith('http')) {
                setUrl(queryPath);
            } else {
                // Ensure path starts with slash if not present, and handle BACKEND_SYSTEM_API
                // If queryPath includes BACKEND_SYSTEM_API already? Usually not based on logic: /green/path
                const cleanPath = queryPath.startsWith('/') ? queryPath : `/${queryPath}`;

                // If the path passed already includes /green (from env.BACKEND_SYSTEM_API usually), we might double up if not careful.
                // The API View passes `/green${ep.path}`. And env.BACKEND_SYSTEM_API is likely `/green` or full URL.
                // Let's assume input path is intended to be the full resource path relative to domain root, OR full url.

                // If env.BACKEND_SYSTEM_API is a relative path (e.g. /green), and queryPath is /green/users/1
                // We should just use window.origin + queryPath or similar.

                // But existing setUrl uses `${env.BACKEND_SYSTEM_API}/`.
                // Let's rely on standard logic: if it starts with http, use it.
                // If not, pre-pend origin if missing?
                // Actually existing default is `${env.BACKEND_SYSTEM_API}/`
