    ...
    <div className="space-y-4 animate-in fade-in duration-500">
        ...
        <div className="flex flex-row items-center gap-3">
          ...
          {history.length > 0 && (
            <Button
              ...
              className="lowercase text-rose-500 hover:text-rose-600 hover:bg-rose-500/5"
            >
      ...
      {showHistory && history.length > 0 && (
        <Card className="overflow-hidden bg-muted/40 animate-in slide-in-from-top-2 duration-300">
          <div className="p-4 border-b border-border/5 bg-background/50">
            <p className="text-sm font-medium text-muted-foreground lowercase px-1">
              recent activity
            </p>
          </div>
          <div className="divide-y divide-border/5 max-h-56 overflow-y-auto scrollbar-none">
            {history.map((item) => (
              ...
              >
                ...
                <span className="text-muted-foreground truncate flex-1 font-normal group-hover:text-foreground transition-colors">
                  {item.url}
                </span>
                <div
                  className={cn(
                    'rounded-lg text-xs font-bold px-2 py-0.5 lowercase',
      ...
      <Card className="overflow-hidden bg-card">
        <div className="p-4 sm:p-5 space-y-5">
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            ...
            >
              {loading ? (
                ...
                <Icons.play className="size-4 mr-2" />
              )}
          ...
          <div className="p-5 bg-muted/40 rounded-xl border-none space-y-4">
            <div className="flex flex-row items-center gap-3">
              ...
              <div className="px-2.5 py-0.5 text-[11px] font-bold rounded-lg bg-background text-foreground ml-auto lowercase">
                {L.labels.autoInjected || 'auto-sync'}
              </div>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-1.5">
                <label className="text-sm font-medium text-muted-foreground lowercase px-1 ml-1">
        ...
        <div className="flex border-t border-border/5 bg-muted/10 overflow-hidden">
          {['headers', ...(method !== 'GET' ? ['body'] : [])].map((tab) => (
            ...
            >
              {tab === 'headers'
                ? `${L.labels.headers || 'headers'} (${headers.filter((h) => h.enabled).length})`
                : L.labels.body || 'body data'}
            </button>
          ))}
        </div>
          ...
          {requestTab === 'headers' && (
            <div className="space-y-2">
              {headers.map((header, index) => (
                <div
                  key={index}
                  className="flex flex-row items-center gap-3 p-2 bg-muted/20 rounded-xl border-none group"
                >
                  <input
                    type="checkbox"
                  ...
                  <button
                    ...
                    className="p-2 text-muted-foreground hover:text-rose-500 opacity-0 group-hover:opacity-100 transition-all mr-2"
                  >
          ...
          <div className="p-4 sm:p-5 border-b border-border/5 bg-muted/40 flex items-center justify-between">
            <div className="flex items-center gap-4 px-2">
              ...
              {responseStatus !== null && (
                <div
                  className={cn(
                    'rounded-lg text-sm font-bold px-3 py-1 lowercase',
      ...
      {!response && !loading && (
        <Card className="p-16 text-center bg-muted/20 transition-all">
          ...
          <p className="text-lg font-medium text-muted-foreground lowercase mb-2">
            {L.labels.readyToTest || 'no data detected'}
          </p>
          <p className="text-base text-muted-foreground lowercase max-w-sm mx-auto">
            {L.labels.readyToTestDesc ||
