...
import { MODULE_LABELS, API_STATUS } from '@cp/config';
...
export function useErrorTemplates() {
    ...
    const handleSave = async (statusCode: number, template: string) => {
        try {
            const res = await fetchWithCsrf(API.save, {
                method: 'POST',
    ...
    const handleDelete = async (id: number) => {
        ...
        try {
            const res = await fetchWithCsrf(API.delete(id), { method: 'DELETE' });
            const data = await res.json();
            if (data.status === API_STATUS.SUCCESS) {
