      ...
      if (r.handler_config) {
        try {
          ...
          if (cfg.dataSourceId && docs.schemas[cfg.dataSourceId]) {
            const dsName =
              dataSources.find((d: any) => d.id === cfg.dataSourceId)?.table_name || 'any';
            if (method === 'POST')
              reqBodySchema = `Omit<${dsName}, 'id' | 'created_at' | 'updated_at'>`;
                    ...
                    <div className="mb-4">
                      {Object.entries(
                        ...
                        <div key={category} className="mb-4">
                          ...
                          {routes.map((r: any) => {
                            const globalIdx = allRoutes.findIndex(
                              (ar) => ar.id === r.id && ar.route_path === r.route_path,
                            );
                            const method = (r.method || 'GET').toUpperCase();
                            const path = r.route_path || r.endpoint || '/';
                            ...
                            return (
                              ...
                              >
                                ...
                                />
                                <label
                                  htmlFor={`route-${r.id || path}`}
                                  className="flex flex-col gap-1 cursor-pointer w-full overflow-hidden"
                                >
