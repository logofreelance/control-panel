...
export default function InstallPage() {
    ...
    useEffect(() => {
        ...
        const checkSystem = async () => {
            ...
            const targetUrl = `${env.API_BASE}/system-status`;
            setDebugInfo({
    ...
    useEffect(() => {
        ...
        const timer = setTimeout(async () => {
            ...
            try {
                const res = await fetchWithCsrf(`${env.API_BASE}/validate-db-url`, {
                    method: 'POST',
    ...
    const handleDbSetup = async (e: React.FormEvent) => {
        ...
        try {
            const res = await fetchWithCsrf(`${env.API_BASE}/setup-db`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ dbUrl }),
            });
    ...
    const handleCreateAdmin = async (e: React.FormEvent) => {
        ...
        try {
            const res = await fetchWithCsrf(`${env.API_BASE}/install`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
