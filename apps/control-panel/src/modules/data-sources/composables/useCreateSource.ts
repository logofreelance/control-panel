/**
 * data-sources/composables/useCreateSource.ts
 * 
...
/**
 * Hook for managing data source creation
...
export function useCreateSource() {
    ...
    useEffect(() => {
        const fetchTemplates = async () => {
            try {
                ...
                const response = await apiClient.get<Template[]>(`${api.dataSources.list}/templates`);
    ...
    const validate = useCallback(async (
        ...
        try {
            ...
            const response = await apiClient.post<ValidationResult>(api.dataSources.validate, {
                tableName,
    ...
    // Create data source
    const create = useCallback(async (payload: CreateSourcePayload): Promise<DataSource | null> => {
        // Client-side validation using msg from context
        if (!payload.name.trim()) {
            addToast(msg.dataSources.validation.resourceNameRequired, TOAST_TYPE.ERROR);
            return null;
        }
        if (!payload.tableName.trim()) {
            addToast(msg.dataSources.validation.tableNameRequired, TOAST_TYPE.ERROR);
            return null;
        }
        if (payload.schema.columns.length === 0) {
            addToast(msg.dataSources.validation.minOneColumn, TOAST_TYPE.ERROR);
            return null;
        }
        ...
        try {
            // ✅ Use api from context
            const response = await apiClient.post<DataSource>(api.dataSources.create, payload);
            if (response.status === API_STATUS.SUCCESS && response.data) {
                // ✅ Use msg from context
                addToast(msg.dataSources.success.sourceCreated, TOAST_TYPE.SUCCESS);
    ...
    return {
        ...
        defaultOptions: {
            timestamps: defaults.dataSources.schema.timestamps,
            softDelete: defaults.dataSources.schema.softDelete,
        },
    };
}
