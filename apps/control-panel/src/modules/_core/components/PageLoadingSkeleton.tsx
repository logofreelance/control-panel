 ...
 * 
 * PURE DI: Uses @cp/config labels
 */

import { LABELS } from '@cp/config';
    ...
    <div className="space-y-8 animate-pulse">
        ...
        {showStats && (
            <section className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {[1, 2, 3, 4].map((i) => (
                    <div key={i} className="bg-white rounded-[1.5rem] p-6 border border-slate-100 shadow-sm">
        ...
        <div className={`grid gap-8 ${showSidebar ? 'grid-cols-1 lg:grid-cols-3' : 'grid-cols-1'}`}>
            {/* Main Content */}
            <div className={`bg-white rounded-[2rem] border border-slate-100 shadow-sm overflow-hidden ${showSidebar ? 'lg:col-span-2' : ''}`}>
                {/* Toolbar */}
            ...
            {showSidebar && (
                <div className="space-y-6">
                    <div className="bg-slate-900 rounded-[2rem] p-6 h-56" />
                    <div className="bg-white rounded-[2rem] p-6 h-48 border border-slate-100 shadow-sm" />
                </div>
            )}
        </div>
