 ...
 * 
 * 🤖 AI INSTRUCTIONS:
 * - NEVER import from module config/api or @cp/config directly in components
...
import {
    ...
    MODULE_LABELS,
    COMMON_LABELS,
} from '@cp/config/client';

// Import module APIs (only here - single injection point)
import { API as DataSourcesAPI } from '@/modules/data-sources';
...
interface ModuleApiRegistry {
    dataSources: typeof DataSourcesAPI;
    // Future modules:
...
interface LabelsRegistry {
    mod: typeof MODULE_LABELS;      // Module-specific labels (dataSources, etc.)
 ...
 * 
 * // Use defaults
 * const limit = defaults.dataSources.resourceForm.defaultLimit;
 * 
 * // Use messages
 * addToast(msg.dataSources.success.sourceCreated, TOAST_TYPE.SUCCESS);
 * 
 * // Use API endpoints
 * await apiClient.get(api.dataSources.list);
 * 
 * // Use icons
