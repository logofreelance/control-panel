/**
 * data-sources/composables/useColumnBuilder.ts
 * 
 * Column list management using @cp/frontend-list-builder
 * 
...
export function useColumnBuilder(options: UseColumnBuilderOptions = {}) {
    ...
    const createColumn = useMemo(() => (): ColumnDefinition => ({
        name: '',
        type: defaults.dataSources.columnBuilder.defaultType,
        required: false,
        unique: false,
    }), [defaults]);
    ...
    const validate = (): { valid: boolean; errors: string[] } => {
        ...
        list.items.forEach((col, index) => {
            if (!col.name.trim()) {
                // ✅ Use msg from context
                errors.push(`${msg.dataSources.validation.columnNameRequired} (Column ${index + 1})`);
            }
            if (!col.type) {
                errors.push(`${msg.dataSources.validation.columnTypeRequired} (Column ${index + 1})`);
            }
            const typeInfo = getTypeInfo(col.type);
            if (typeInfo?.requiresValues && (!col.values || col.values.length === 0)) {
                errors.push(msg.dataSources.validation.columnValuesRequired(col.name, typeInfo.label));
            }
            if (typeInfo?.requiresTarget && !col.target) {
                errors.push(msg.dataSources.validation.columnTargetRequired(col.name, typeInfo.label));
            }
        });
        ...
        if (duplicates.length > 0) {
            errors.push(msg.dataSources.validation.duplicateColumnNames([...new Set(duplicates)]));
        }
