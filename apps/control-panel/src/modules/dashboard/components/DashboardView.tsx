...
// PURE DI: Uses @cp/config labels/icons and composables
// Loading handled by local Solid Overlay states for premium feel

import { Badge, Button } from '@cp/ui';
import { Icons, MODULE_LABELS, COMMON_LABELS } from '@cp/config/client';
...
export const DashboardView = () => {
    ...
    const { stats, nodes, copyGatewayUrl, gatewayUrl, loading } = useDashboard();
        ...
        <div className="space-y-6">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                ...
                <div className="flex items-center gap-2.5 bg-white px-3 py-1.5 rounded-lg shadow-sm shadow-slate-200/50">
                    <div className={`w-1.5 h-1.5 rounded-full animate-pulse ml-1 ${nodes?.some(n => n.status === 'online') ? 'bg-emerald-500' : 'bg-amber-500'}`}></div>
                    <span className="text-[10px] font-medium text-slate-500">
                        {nodes?.some(n => n.status === 'online') ? L.liveStatus : 'Menunggu Instance'}
                    </span>
                </div>
            </div>
            ...
            <div className="relative min-h-[140px]">
                ...
                <section className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4">
                    {/* Active Cluster Nodes (Replaced Total Users) */}
                    <div className="bg-white rounded-xl p-5 shadow-sm shadow-slate-200/50 hover:shadow-md hover:shadow-slate-200/50 transition-all group">
                        <div className="flex justify-between items-start mb-3">
                            <p className="text-xs font-medium text-slate-500">Backend System Nodes</p>
                            <div className="w-7 h-7 rounded-lg bg-blue-50 flex items-center justify-center text-blue-600">
                                <Icons.globe className="w-3.5 h-3.5" />
                            </div>
                        </div>
                        <div>
                            <p className="text-2xl font-semibold tracking-tight text-slate-900">{nodes?.filter(n => n.status === 'online').length || 0}</p>
                            <p className="text-[10px] font-normal text-slate-400 mt-1">Aktif & Sinkron</p>
                        </div>
                    </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div className="lg:col-span-2 bg-white rounded-xl shadow-sm shadow-slate-200/50 overflow-hidden flex flex-col">
                        <div className="p-5 border-b border-slate-50 flex items-center justify-between">
                            ...
                            <Badge variant="slate" className="px-2 py-0.5 text-[10px] font-medium bg-slate-50 text-slate-600">{L.labels.liveStream}</Badge>
                        </div>
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="p-5 bg-slate-900 rounded-xl text-white shadow-lg shadow-slate-900/10 relative overflow-hidden group">
                            ...
                            <h3 className="font-semibold text-sm mb-4 relative z-10 flex items-center gap-2">
                                <Icons.server className="w-4 h-4 text-emerald-400" />
                                Cluster Nodes ({nodes?.length || 0})
                            </h3>
                            <div className="space-y-3 relative z-10">
                                {nodes?.length === 0 && (
                                    <div className="p-4 bg-white/5 rounded-lg text-center border border-white/10 border-dashed">
                                        <p className="text-[10px] text-slate-400">Belum ada Node Backend yang terdeteksi.</p>
                                    </div>
                                )}
                                {nodes?.map((node, i) => (
                                    <div key={i} className={`p-3 rounded-lg border transition-colors ${node.status === 'online' ? 'bg-white/5 border-emerald-500/20 hover:border-emerald-500/40' : 'bg-red-900/20 border-red-500/20'}`}>
                                        <div className="flex items-center justify-between mb-2">
                                            <div className="flex items-center gap-2">
                                                <div className={`w-2 h-2 rounded-full ${node.status === 'online' ? 'bg-emerald-500 animate-pulse' : 'bg-red-500'}`}></div>
                                                <span className="text-xs font-medium font-mono text-slate-200">{node.nodeId.substring(0,8)}</span>
                                            </div>
                                            <span className={`text-[9px] px-1.5 py-0.5 rounded font-medium ${node.status === 'online' ? 'bg-emerald-500/20 text-emerald-400' : 'bg-red-500/20 text-red-400'}`}>
                                        ...
                                        <div className="flex items-center justify-between text-[10px] text-slate-400 mb-1.5">
                                            ...
                                            <span title="Uptime">{Math.floor(node.uptime / 60)}m</span>
                                        </div>
                                        
                                        {/* Telemetry Sparklines (simplified text array) */}
                                        <div className="grid grid-cols-2 gap-2 mt-2 pt-2 border-t border-white/5">
                                            <div>
                                                <p className="text-[8px] text-slate-500 uppercase tracking-wider mb-0.5">CPU</p>
                                                <p className="text-[10px] font-mono text-slate-300">{node.cpuUsage}</p>
                                            </div>
                                            <div>
                                                <p className="text-[8px] text-slate-500 uppercase tracking-wider mb-0.5">RAM</p>
                                                <p className="text-[10px] font-mono text-slate-300">{node.memoryUsage}</p>
                                            </div>
                                        </div>
                                    </div>
                                ))}
                        ...
                        <div className="p-5 bg-white rounded-xl shadow-sm shadow-slate-200/50 overflow-hidden relative group">
                            <div className="flex items-center gap-3 mb-4">
                                ...
                                <div>
                                    <h4 className="font-semibold text-sm text-slate-900">Active Gateway</h4>
                                    <p className="text-[10px] text-slate-500 font-normal">Titik akses utama jaringan</p>
                                </div>
                            </div>
                            <div className="bg-slate-50 rounded-lg p-3 mb-4">
                                <code className="text-[10px] font-mono text-slate-600 break-all leading-relaxed block bg-white p-2 rounded shadow-sm">
                                    {nodes?.find(n => n.status === 'online')?.endpointUrl || gatewayUrl}
                                </code>
                            </div>
                            <Button
                                onClick={copyGatewayUrl}
                                disabled={!nodes?.some(n => n.status === 'online')}
                                className="w-full py-2.5 text-xs font-medium shadow-none rounded-lg gap-2 bg-slate-900 text-white hover:bg-slate-800 disabled:opacity-50"
                            >
                                <Icons.copy className="w-3.5 h-3.5" /> Salin URL API
                            </Button>
                        </div>
                    </div>
                </div>
